...
<table border="1"> 
	...
	<c:forEach items="${requestScope.recipientAll}" var="data">
		 <tr>
		 	<td><a href="${pageContext.request.contextPath}/probono/recipient?recipientId=${data.id}">${data.id}</a></td>
		 	<!-- "${url}recipient?recipientID=${data.id}" -->
		 	<!-- ${pageContext.request.contextPath}/probono/recipient?recipientId=${data.id} -->
		 	<td>${data.name}</td>
